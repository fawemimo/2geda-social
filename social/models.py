...
# Activity feed / inbox record for a user.
...
class Notification(UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    actor = models.ForeignKey(
        ...
        null=True,  # system notifications have no actor
        blank=True,
    )
    ...
    # Source object (optional; e.g. the liked post, the new follow)
    content_type = models.ForeignKey(ContentType, null=True, blank=True, on_delete=models.SET_NULL)
    object_id = models.UUIDField(null=True, blank=True)
    source_object = GenericForeignKey("content_type", "object_id")

    # Message body (rendered server-side for push; re-rendered client-side for display)
    ...
    class Meta:
        ...
        ordering = ["-created_at"]
        indexes = [
            # Unread notifications for user X — the hot path
            models.Index(
            ...
            # Full inbox
            models.Index(fields=["recipient", "-created_at"], name="notif_inbox_idx"),
            BrinIndex(fields=["created_at"], name="notif_created_brin_idx"),
        ]
