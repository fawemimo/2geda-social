...
class EmailService:
    ...
    def _resolve_template(template_name):
        """Accept "otp", "otp.txt" or "mails/otp.txt" and normalise to a template path."""
    ...
    def send_email(self, *args, **kwargs):
        """Public entry point. Returns the Resend response dict, or None on failure."""
        return self._send_email(*args, **kwargs)

    def _send_email(self, to: list[str], obj, from_email: str = "2geda Social App", other_values=None, subject=None):
        ...
        try:
            ...
            logger.error(f"Failed to send email: {e} - {body}")
